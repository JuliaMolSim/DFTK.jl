...
function compute_forces_estimate(basis, vec, φ, Pks)
    ...
    for (iel, (el, positions)) in enumerate(atoms)
        ...
        for (ir, r) in enumerate(positions)
            f_est[iel][ir], cs = force_estimate(basis, φ, vec, Pks, form_factors, r)
        end
    end
    f_est, cs
end

function force_estimate(basis, φ, vec, Pks, form_factors, r)
    ...
    plot(vec_sort, label="Mvec")
    xlabel("index of G by increasing norm")
    legend()

    figure()
    plot(v[sortperm(normG)], label="M∇Vφ")
    xlabel("index of G by increasing norm")
    legend()

    # cumsum
    cs = real.(conj.(v) .* vec[1] )
