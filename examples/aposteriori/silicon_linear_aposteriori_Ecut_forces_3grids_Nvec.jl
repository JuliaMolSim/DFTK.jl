...
diff_list_res_loc = zeros((K,K))
diff_list_res_nonloc = zeros((K,K))
...
for Ecut_g in Ecut_list
    ...
    for Ecut_f in [Ecut_ref]
        ...
        f_res_nonloc, cs = compute_forces_estimate(basis_f, Merr, φr, Pks, occupation; term="nonlocal")
        ...
        diff_list_res_loc[i,j] = abs(f_res_loc[1][2][1])
