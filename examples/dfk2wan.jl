...
# (via `basis`, `scfres` and `kgrid`) the functon asks for a prefix (here "Si") and
...
prefix = "Si"
dftk2wan_win_file(prefix, basis, scfres, kgrid, 8;
...
# preprocessing.
#
# - An SCDM routine for the generation of ``A_{mn}(k)`matrices is still
#   to be implemented.
#
