...
stress, (dstress,) = value_and_pushforward(AutoForwardDiff(), zeros(6), (strain_pattern,)) do voigt_strain
    stress_from_strain(model_scan, voigt_strain; symmetries=symmetries_strain, Ecut, kgrid, tol)

end;

# Note here that the `value_and_pushforward(...) do voigt_strain ... end` syntax defines
# an anonymous function to compute the stress from a given strain, same as `stress_fn` in
# the above example. See the [Julia documentation on the do-block-syntax](https://docs.julialang.org/en/v1/manual/functions/#Do-Block-Syntax-for-Function-Arguments) for details.
...
# We again convert to GPa and print, which are numbers that are in close agreement to PBE.
println("C11: ", uconvert(u"GPa", dstress[1] * u"hartree" / u"bohr"^3))
println("C12: ", uconvert(u"GPa", dstress[2] * u"hartree" / u"bohr"^3))
println("C44: ", uconvert(u"GPa", dstress[4] * u"hartree" / u"bohr"^3))
