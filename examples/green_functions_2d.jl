...
Rmax = 3  # integer; produces a (2*Rmax+1)×(2*Rmax+1) supercell
...
# Define lattice vectors for (2*Rmax+1)×(2*Rmax+1) cells
R_range = -Rmax:Rmax
R_vectors = [[i, j, 0.0] for i in R_range for j in R_range]
...
# Build extended Green's function on (2*Rmax+1)×(2*Rmax+1) grid
nx, ny, nz = basis.fft_size
ncell = 2 * Rmax + 1
G_extended = zeros(ComplexF64, ncell * nx, ncell * ny)
...
for (iR, R) in enumerate(R_vectors)
    ...
    ix_start = (i + Rmax) * nx + 1
    iy_start = (j + Rmax) * ny + 1
    G_extended[ix_start:ix_start+nx-1, iy_start:iy_start+ny-1] = G_R
end

# Create coordinate arrays for extended grid
x_extended = range(-Rmax*a, (Rmax+1)*a, length=ncell*nx)
y_extended = range(-Rmax*a, (Rmax+1)*a, length=ncell*ny)
