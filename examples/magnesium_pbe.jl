...
#
a = 1.5970245 * mg.units.ang_to_bohr
b = 2.766127574932283 * mg.units.ang_to_bohr
c = 5.171978 * mg.units.ang_to_bohr
A = mg.ArrayWithUnit([[-a -b  0]; [-a  b  0]; [0   0 -c]], "bohr")
lattice = mg.lattice.Lattice(A)
structure = mg.Structure(lattice, ["Mg", "Mg"],
...
Mg = Species(mg.Element("Mg").number, psp=load_psp("mg-pade-q2.hgh"))
composition = [Mg => [s.frac_coords for s in structure.sites if s.species_string == "Mg"]]
...
# TODO Use the routine to compute this properly
efermi = 0.5
