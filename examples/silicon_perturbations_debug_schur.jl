...
for α in α_list
    global Ecut
    Ep_fine, ψp_fine, ρp_fine, egvalp2, egvalp3, egvalp_rr, forcesp, Ep2_fine = perturbation(basis, kcoords, ksymops, scfres, α*Ecut; compute_forces=compute_forces, schur=true)
    ...
    nrj2_fine = abs(sum(values(Ep2_fine)) - sum(values(scfres_ref.energies)))
...
semilogy(α_list, nrj2_fine_list, label="nrj2_fine")
legend()
