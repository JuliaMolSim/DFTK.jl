...
mutable struct CTPlan{T, forward, Tt, Tn} <: Plan{T}
    ...
    pinv::ScaledPlan{T}
    CTPlan{T, forward, Tt, Tn}(n, tsteps, nstep) where {T, forward, Tt, Tn} =
        new(n, tsteps, nstep)
end

summary(p::CTPlan{T, forw}) where {T, forw} =
    string(forw ? "for" : "back", "ward CTPlan{$T} of size ", p.n)
...
invCT(p::CTPlan{T, forward, Tt, Tn}) where {T, forward, Tt, Tn} =
    CTPlan{T, !forward, Tuple{map(inv, Tt.parameters)...}, inv(Tn)}(
        p.n, map(inv, p.tsteps), inv(p.nstep)
    )

plan_inv(p::CTPlan{T}) where {T} = ScaledPlan(invCT(p), normalization(real(T), p.n, 1))
...
applystep(
    ...
    y0::Integer,
    ys::Integer,
) where {T, n} = applystep(ns, r, x, x0, xs * r, xs, y, y0, ys, ys * n)
...
inv(::Type{NontwiddleKernelStep{T, n, forward}}) where {T, n, forward} =
    NontwiddleKernelStep{T, n, !forward}
show(io::IO, ns::NontwiddleKernelStep{T, n}) where {T, n} = print(io, "size ", n, " kernel")
...
function applystep(p::CTPlan{T}, x, x0, xs, y, y0, ys, step::Int) where {T}
...
function twiddle(
    T::Type{Complex{Tr}}, ω_n, forward::Bool, n::Integer, k::Integer, x
) where {Tr <: Real}
    k == 0 && return x
    2k == n && return :(-$x)
    if 4k == n || 4k == 3n
        ...
        if (4k == n && !forward) || (4k == 3n && forward) # ω = +im
            return :(
                let $tmpvar = $x
                    complex(-imag($tmpvar), real($tmpvar))
                end
            )
        else  # ω = -im
            return :(
    ...
    if 8k == n || 8k == 3n || 8k == 5n || 8k == 7n # ω = [(1,-1), (-1,-1), (-1,1), or (1,1)] * sqrt(0.5)
        ...
        if (8k == n && forward) || (8k == 7n && !forward)
            return :(
                let $tmpvar = $x
                    $sqrthalf *
                    complex(real($tmpvar) + imag($tmpvar), imag($tmpvar) - real($tmpvar))
                end
            )
        elseif (8k == 3n && forward) || (8k == 5n && !forward)
            return :(
                let $tmpvar = $x
                    $sqrthalf *
                    complex(imag($tmpvar) - real($tmpvar), -real($tmpvar) - imag($tmpvar))
                end
            )
        elseif (8k == 5n && forward) || (8k == 3n && !forward)
            return :(
                let $tmpvar = $x
                    $sqrthalf *
                    complex(-real($tmpvar) - imag($tmpvar), real($tmpvar) - imag($tmpvar))
                end
            )
...
function dftgen(T, ω_n, forward::Bool, n::Integer, x, y)
    n == 1 && return :($(y(0)) = $(x(0)))
    tmpvars = Symbol[gensym(string("dftgen_", j)) for j = 0:n-1]
    ...
    n == 2 && return Expr(
        ...
        Expr(
            :block,
            :($(y(0)) = $(tmpvars[1]) + $(tmpvars[2])),
            :($(y(1)) = $(tmpvars[1]) - $(tmpvars[2])),
        ),
    )
    # JuliaLang/julia #21774
    Expr(
                ...
                :(
                    $(y(k)) =
                        $(Expr(
                            ...
                            :+,
                            [
                                twiddle(T, ω_n, forward, n, j * k, tmpvars[j+1])
                                for j = 0:n-1
                            ]...,
                        ))
                ) for k = 0:n-1
...
function fftgen(T, ω_n, forward::Bool, n::Integer, x, y)
    ...
    z = Symbol[gensym(string("fftgen_", j1, "_", k2)) for j1 = 0:r-1, k2 = 0:m-1]
    ...
    if gcd(r, m) == 1 # radix-r Prime-Factor (PFA) step
        ...
        Fm = Expr[
            fftgen(T, ω_n, forward, m, j2 -> x(mod(r * j2 + m * j1, n)), k2 -> z[j1+1, k2+1])
            for j1 = 0:r-1
        ]
        # get expressions to perform m sub-FFTs of length r
        Fr = Expr[
            fftgen(
                T, ω_n, forward, r, j1 -> z[j1+1, k2+1], k1 -> y(mod(mi * k1 + ri * k2, n))
            ) for k2 = 0:m-1
        ]
    else # radix-r Cooley-Tukey step
        # get expressions to perform r sub-FFTs of length m
        Fm = Expr[
            fftgen(T, ω_n, forward, m, j2 -> x(r * j2 + j1), k2 -> z[j1+1, k2+1])
            for j1 = 0:r-1
        ]
        # get expressions to perform m sub-FFTs of length r
        Fr = Expr[
...
fftgen(T, ω_n, forward::Bool, n::Integer, X::Symbol, Y::Symbol) =
    fftgen(T, ω_n, forward, n, j -> :($X[$(j + 1)]), k -> :($Y[$(k + 1)]))
...
macro nontwiddle(args...)
    ...
    @assert isa(T, Type)
    quote
        function FourierTransforms.applystep(
        ...
        function FourierTransforms.applystep(
            ...
            @inbounds @simd for i = 0:vn-1
                $(fftgen(
                    ...
                    j ->
                        j == 0 ? :(X[(x0+xvs*i)+xs*$j]) :
                        forward ? :(W[$j, i+1] * X[(x0+xvs*i)+xs*$j]) :
                        :(mulconj(W[$j, i+1], X[(x0+xvs*i)+xs*$j])),
                    j -> :(X[(x0+xvs*i)+xs*$j]),
                ))
            end
            X
...
const fft_kernel_sizes_sorted = sort!(Int[n for n in fft_kernel_sizes]; rev=true)
...
show(io::IO, s::NontwiddleBluesteinStep) = print(io, "size ", s.n, " Bluestein-", s.n2)

function applystep(
    ns::NontwiddleBluesteinStep{T},
    r,
    x::AbstractArray{T},
    x0,
    xs,
...
show(io::IO, s::TwiddleBluesteinStep) = print(io, "radix ", s.r, " Bluestein-", s.r2)
...
Nontwiddle(T, n::Int, forw::Bool; ωpow=ωpow) =
    (T <: CTComplex && n in fft_kernel_sizes) || n in generic_kernel_sizes ?
    NontwiddleKernelStep{T, n, forw}() : NontwiddleBluesteinStep{T}(n, ωpow, forw)
Twiddle(T, n::Int, r::Int, forw::Bool; ωpow=ωpow) =
    (T <: CTComplex && r in fft_kernel_sizes) || r == 4 || r == 2 ?
    TwiddleKernelStep{T, r, forw}(n, ωpow) : TwiddleBluesteinStep{T}(n, ωpow, r, forw)
