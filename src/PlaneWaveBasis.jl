...
@timing function build_kpoints(
    model::Model{T}, fft_size, kcoords, Ecut; variational=true
) where {T}
...
@timing function PlaneWaveBasis(
    model::Model{T},
    Ecut::Number,
    kcoords::AbstractVector,
    ksymops,
    symops=nothing;
    fft_size=nothing,
    variational=true,
    ...
    if variational && optimize_fft_size
        ...
        fft_size = determine_fft_size_precise(
            model.lattice, Ecut, kpoints; supersampling=supersampling
        )
        fft_size = Tuple{Int, Int, Int}(fft_size)
    end
    ...
    if variational && Ecut > max_E
        @warn(
            "For a variational method, Ecut should be less than the maximal kinetic energy " *
...
"""
function PlaneWaveBasis(
    basis::PlaneWaveBasis, kcoords::AbstractVector, ksymops::AbstractVector, symops=nothing
)
    PlaneWaveBasis(basis.model, basis.Ecut, kcoords, ksymops, symops; fft_size=basis.fft_size)
end
...
function index_G_vectors(
    basis::PlaneWaveBasis, kpoint::Kpoint, G::AbstractVector{T}
) where {T <: Integer}
...
"""
@timing_seq function G_to_r!(
    f_real::AbstractArray3, basis::PlaneWaveBasis, f_fourier::AbstractArray3
)
    mul!(f_real, basis.opIFFT, f_fourier)
end
@timing_seq function G_to_r!(
    f_real::AbstractArray3, basis::PlaneWaveBasis, kpt::Kpoint, f_fourier::AbstractVector
)
...
"""
@timing_seq function r_to_G!(
    f_fourier::AbstractArray3, basis::PlaneWaveBasis, f_real::AbstractArray3
)
    mul!(f_fourier, basis.opFFT, f_real)
end
@timing_seq function r_to_G!(
    f_fourier::AbstractVector, basis::PlaneWaveBasis, kpt::Kpoint, f_real::AbstractArray3
)
    @assert size(f_real) == basis.fft_size
    @assert length(f_fourier) == length(kpt.mapping)
...
function r_to_G_matrix(basis::PlaneWaveBasis{T}) where {T}
    ...
    for (iG, G) in enumerate(G_vectors(basis))
        for (ir, r) in enumerate(r_vectors(basis))
            ret[iG, ir] =
                cis(-2π * dot(r, G)) * sqrt(basis.model.unit_cell_volume) /
                prod(basis.fft_size)
        end
    end
    ret
...
function PlaneWaveBasis(basis::PlaneWaveBasis; use_symmetry)
    ...
    new_basis = PlaneWaveBasis(
        basis.model,
        basis.Ecut,
        kcoords,
        [[identity_symop()] for _ = 1:length(kcoords)];
        fft_size=basis.fft_size,
    )
end
