...
function PlaneWaveBasis(model::Model{T},
                        ...
                        kcoords::AbstractVector, kweights, kgrid,
                        kshift, symmetries, comm_kpts) where {T <: Real}
    ...
    @assert mpi_sum(sum(kweights_thisproc), comm_kpts) ≈ sum(kweights) ≈ model.n_spin_components
