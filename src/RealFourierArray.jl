...
## these functions are minimally optimized for the moment
...
# Tr is the underlying real type
# T is the type of the array in real space
# TODO make this concrete by parametrizing on the type of the arrays
mutable struct RealFourierArray{Tr <: Real, T <: Union{Tr, Complex{Tr}}}
    basis::PlaneWaveBasis{Tr}
    _real::AbstractArray{T, 3}
    _fourier::AbstractArray{Complex{Tr}, 3}
    _state::Int # 0: only _real set. 1: only _fourier set. 2: both set
end
# Type of the real part
Base.eltype(A::RealFourierArray{Tr, T}) where {Tr, T} = T
...
# TODO optimize those a bit better
