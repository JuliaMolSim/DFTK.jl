...
apart (in inverse Bohrs).  The `kshift` keyword argument allows to specify an explicit
shift for all ``k``-points. See [Performing a convergence study](@ref) for some practical
recommendations how to choose a reasonable spacing.
