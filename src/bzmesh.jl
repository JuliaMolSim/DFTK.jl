...
See also [`KgridDensity`](@ref) and [`KgridMinimalNumber`](@ref)
...
struct KgridDensity <: AbstractKgridGenerator
    spacing::Float64
    kshift::Vec3{Rational{Int}}
end
...
"""
function KgridDensity(spacing; kshift=[0, 0, 0])
    KgridDensity(austrip(spacing), rationalize.(kshift; tol=1e-6))
end
KgridDensity(spacing::Quantity, kshift) = KgridDensity(austrip(spacing); kshift)

function build_kgrid(lattice::AbstractMatrix, kgen::KgridDensity)
...
function KgridMinimalNumber(n_kpoints; kshift=[0, 0, 0])
    KgridDensity(n_kpoints, rationalize.(kshift; tol=1e-6))
end
...
Note that this function is deprecated. Use [`KgridDensity`](@ref) instead.
"""
function kgrid_from_maximal_spacing(system::AbstractSystem, spacing; kshift=[0, 0, 0])
    @warn("kgrid_from_maximal_spacing(system, spacing; kshift) is now deprecated. " *
          "Now you should pass the `KgridDensity(spacing; kshift)` directly " *
          "as `PlaneWaveBasis(model; kgrid=KgridDensity(spacing; kshift), ...)` or " *
          "use `build_kgrid(lattice, KgridDensity(spacing; kshift))`")
    pseudopotentials = fill(nothing, length(system))
    kgen = KgridDensity(spacing, kshift)
...
function kgrid_from_maximal_spacing(lattice::AbstractMatrix, spacing; kshift=[0, 0, 0])
    @warn("kgrid_from_maximal_spacing(system, spacing; kshift) is now deprecated. " *
          ...
          "as `PlaneWaveBasis(model; kgrid=KgridDensity(spacing; kshift), ...)` or " *
          "use `build_kgrid(lattice, KgridDensity(spacing; kshift))`")
    build_kgrid(lattice, KgridDensity(spacing, kshift))
end
