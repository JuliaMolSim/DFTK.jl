...
See also [`KgridMaximalSpacing`](@ref) and [`KgridMinimalNumber`](@ref)
...
struct KgridMaximalSpacing <: AbstractKgridGenerator
    spacing::Float64
    kshift::Vec3{Rational{Int}}
end
...
"""
function KgridMaximalSpacing(spacing; kshift=[0, 0, 0])
    KgridMaximalSpacing(austrip(spacing), rationalize.(kshift; tol=1e-6))
end
KgridMaximalSpacing(spacing::Quantity, kshift) = KgridMaximalSpacing(austrip(spacing); kshift)

function build_kgrid(lattice::AbstractMatrix, kgen::KgridMaximalSpacing)
...
`build_kgrid(lattice, kgen)` where `kgen` is this object.
"""
struct KgridMinimalNumber <: AbstractKgridGenerator
    n_kpoints::Int
    kshift::Vec3{Rational{Int}}
end
function KgridMinimalNumber(n_kpoints; kshift=[0, 0, 0])
    KgridMaximalSpacing(n_kpoints, rationalize.(kshift; tol=1e-6))
end

function build_kgrid(lattice::AbstractMatrix, kgen::KgridMinimalNumber)
    lattice = austrip.(lattice)
    n_dim   = count(!iszero, eachcol(lattice))
    @assert kgen.n_kpoints > 0
...
function kgrid_from_maximal_spacing(system::AbstractSystem, spacing; kshift=[0, 0, 0])
    @warn("kgrid_from_maximal_spacing(system, spacing; kshift) is now deprecated. " *
          "Now you should pass the `KgridMaximalSpacing(spacing; kshift)` directly " *
          "as `PlaneWaveBasis(model; kgrid=KgridMaximalSpacing(spacing; kshift), ...)` or " *
          "use `build_kgrid(lattice, KgridMaximalSpacing(spacing; kshift))`")
    pseudopotentials = fill(nothing, length(system))
    kgen = KgridMaximalSpacing(spacing, kshift)
...
function kgrid_from_maximal_spacing(lattice::AbstractMatrix, spacing; kshift=[0, 0, 0])
    @warn("kgrid_from_maximal_spacing(system, spacing; kshift) is now deprecated. " *
          ...
          "as `PlaneWaveBasis(model; kgrid=KgridMaximalSpacing(spacing; kshift), ...)` or " *
          "use `build_kgrid(lattice, KgridMaximalSpacing(spacing; kshift))`")
    build_kgrid(lattice, KgridMaximalSpacing(spacing, kshift))
end
...
function kgrid_from_minimal_n_kpoints(system::AbstractSystem, n_kpoints::Integer; kshift=[0, 0, 0])
    @warn("kgrid_from_minimial_n_kpoints(system, n_kpoints; kshift) is now deprecated. " *
          "Now you should pass the `KgridMinimalNumber(n_kpoints; kshift)` directly " *
          "as `PlaneWaveBasis(model; kgrid=KgridMinimalNumber(n_kpoints; kshift), ...)` or " *
          "use `build_kgrid(lattice, KgridMinimalNumber(n_kpoints; kshift))`")
    pseudopotentials = fill(nothing, length(system))
...
function kgrid_from_minimal_n_kpoints(lattice, n_kpoints::Integer; kshift=[0, 0, 0])
    build_kgrid(lattice, KgridMinimalNumber(n_kpoints, kshift))
end
