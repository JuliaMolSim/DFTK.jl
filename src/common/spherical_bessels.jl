...
@inline function sphericalbesselj(::Val{0}, x)
    sin(x) / x
end

@inline function sphericalbesselj(::Val{1}, x)
    (sin(x) - cos(x) * x) / x^2
end

@inline function sphericalbesselj(::Val{2}, x)
    (sin(x) * (3 - x^2) + cos(x) * (-3x)) / x^3
end

@inline function sphericalbesselj(::Val{3}, x)
    (sin(x) * (15 - 6x^2) + cos(x) * (x^3 - 15x)) / x^4
end

@inline function sphericalbesselj(::Val{4}, x)
    (sin(x) * (105 - 45x^2 + x^4) + cos(x) * (10x^3 - 105x)) / x^5
end

@inline function sphericalbesselj(::Val{5}, x)
