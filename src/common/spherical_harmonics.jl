...
[Wikipedia](https://en.wikipedia.org/wiki/Table_of_spherical_harmonics#Real_spherical_harmonics).
"""
function ylm_complex(l::Integer, m::Integer, rvec::AbstractVector{T}) where {T}
    @assert 0 ≤ l
    @assert -l ≤ m ≤ l
    @assert length(rvec) == 3
    x, y, z = rvec
    ...
    if l == 1  # p
        (m ==  0) && return   sqrt(3 / 4T(π)) * z / r
        (m == -1) && return   sqrt(3 / 8T(π)) * (x - im*y) / r
        (m ==  1) && return - sqrt(3 / 8T(π)) * (x + im*y) / r
    end

    if l == 2  # d
        (m == -2) && return   sqrt(15 / 32T(π)) * (x - im*y)^2 / r^2
        (m == -1) && return   sqrt(15 /  8T(π)) * (x - im*y) * z / r^2
        (m ==  0) && return   sqrt( 5 / 16T(π)) * (2z^2 - x^2 - y^2) / r^2
        (m ==  1) && return - sqrt(15 /  8T(π)) * (x + im*y) * z / r^2
        (m ==  2) && return   sqrt(15 / 32T(π)) * (x + im*y)^2 / r^2
    end
    ...
    if l == 3  # f
        (m == -3) && return   sqrt( 35 / 64T(π)) * (x - im*y)^3 / r^3
        (m == -2) && return   sqrt(105 / 32T(π)) * (x - im*y)^2 * z / r^3
        (m == -1) && return   sqrt( 21 / 64T(π)) * (x - im*y) * (4z^2 - x^2 - y^2) / r^3
        (m ==  0) && return   sqrt(  7 / 16T(π)) * z * (2z^2 - 3x^2 - 3y^2) / r^3
        (m ==  1) && return - sqrt( 21 / 64T(π)) * (x + im*y) * (4z^2 - x^2 - y^2) / r^3
        (m ==  2) && return   sqrt(105 / 32T(π)) * (x + im*y)^2 * z / r^3
        (m ==  3) && return - sqrt( 35 / 64T(π)) * (x + im*y)^3 / r^3
