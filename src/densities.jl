...
"""
Compute the partial density δρ = Σi φi*conj(δφi) at the indicated ``k``-Point and return it (in Fourier space).
"""
function compute_partial_density!(ρ, basis, kpt, ψk, δψk, occupation)
    @assert length(occupation) == size(ψk, 2)

    # Build the partial density for this k-Point
    ρk_real = [zeros(eltype(basis), basis.fft_size) for it = 1:Threads.nthreads()]
    ψnk_real = [zeros(complex(eltype(basis)), basis.fft_size) for it = 1:Threads.nthreads()]
    δψnk_real = [zeros(complex(eltype(basis)), basis.fft_size) for it = 1:Threads.nthreads()]
    Threads.@threads for n = 1:size(ψk, 2)
        ψnk = @views ψk[:, n]
        δψnk = @views δψk[:, n]
        tid = Threads.threadid()
        G_to_r!(ψnk_real[tid], basis, kpt, ψnk)
        G_to_r!(δψnk_real[tid], basis, kpt, δψnk)
        ρk_real[tid] .+= occupation[n] .* (conj.(ψnk_real[tid]) .* δψnk_real[tid] +
                                      conj.(δψnk_real[tid]) .* ψnk_real[tid])
    end
    for it = 2:Threads.nthreads()
        ρk_real[1] .+= ρk_real[it]
    end
    ...
    # Check sanity in the density, this time we shoudl have ∫δρ = 0
    T = real(eltype(ρk_real))
    sum_ρ = sum(ρk_real)
    if abs(sum_ρ) > sqrt(eps(T))
        @warn("Mismatch in δρ", sum_ρ=sum_ρ)
    end
...
"""
    compute_density(basis::PlaneWaveBasis, ψ::AbstractVector, δψ::AbstractVector,
                    occupation::AbstractVector)

Compute the density and spin density for a wave function `δψ` from the tangent space at `ψ`,
and discretized on the plane-wave grid `basis`, where the individual k-Points
are occupied according to `occupation`.
`δψ` should be one coefficient matrix per k-Point. If the `Model` underlying the basis
is not collinear the spin density is `nothing`.
"""
@views @timing function compute_density(basis::PlaneWaveBasis, ψ, δψ, occupation)
    n_k = length(basis.kpoints)
    n_spin = basis.model.n_spin_components
    ...
    Threads.@threads for (ikpts, ρaccu) in collect(zip(kpt_per_thread, ρaccus))
        ...
        for ik in ikpts
            kpt = basis.kpoints[ik]
            compute_partial_density!(ρ_k, basis, kpt, ψ[ik], δψ[ik], occupation[ik])
            lowpass_for_symmetry!(ρ_k, basis)
            # accumulates all the symops of ρ_k into ρaccu
            accumulate_over_symmetries!(ρaccu[:, :, :, kpt.spin], ρ_k, basis, basis.ksymops[ik])
        end
