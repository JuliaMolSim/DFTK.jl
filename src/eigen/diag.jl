...
function diagonalize_all_kblocks(eigensolver, ham::Hamiltonian, nev_per_kpoint::Int;
                                 ψguess=nothing, prec_type=PreconditionerTPA,
                                 interpolate_kpoints=true, tol=1e-6, miniter=1, maxiter=100,
    ...
    for (ik, kpt) in enumerate(kpoints)
        ...
        @assert size(ψguessk) == (n_components, n_Gk, nev_per_kpoint)

        prec = nothing
        !isnothing(prec_type) && (prec = prec_type(ham[ik]))
        res = eigensolver(ham[ik], to_composite_σG(basis, kpt, ψguessk);
                          prec, tol, miniter, maxiter, n_conv_check)
        results[ik] = merge(res, (; X=from_composite_σG(basis, kpt, res.X)))
    end
