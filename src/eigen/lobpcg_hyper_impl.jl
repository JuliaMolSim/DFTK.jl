...
@timeit to function RR(X, AX, BX, N)
    F = eigen(Hermitian(boverlap(X, AX, assume_hermitian=true)))  # == Hermitian(X' AX)
    F.vectors[:,1:N], F.values[1:N]
end

import LinearAlgebra: cholesky
@timeit to function cholesky(X::Union{Matrix{ComplexF16}, Hermitian{ComplexF16,Matrix{ComplexF16}}})
...
# magnified
@timeit to function ortho(X; tol=2eps(real(eltype(X))))
    local R
...
@timeit to function final_retval(X, AX, resid_history, niter, n_matvec)
    λ = real(diag(X' * AX))
...
@timeit to function LOBPCG(A, X, B=I, precon=((Y, X, R)->R), tol=1e-10, maxiter=100;
