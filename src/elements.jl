...
AtomicPotentials.ionic_charge(el::Element) = nuclear_charge(el)

"""Return the number of valence electrons"""
n_elec_valence(el::Element) = ionic_charge(el)

"""Return the number of core electrons"""
n_elec_core(el::Element) = nuclear_charge(el) - ionic_charge(el)

"""Check presence of model core charge density (non-linear core correction)."""
has_core_density(el::Element) = !isnothing(el.potential.core_charge_density)

"""Check presence of valence charge density (density superposition / projection)."""
has_valence_density(el::Element) = !isnothing(el.potential.valence_density)

"""Check presence of non-local potential."""
has_non_local(el::Element) = !isnothing(el.potential.non_local_potential)

"""Check presence of augmentation charges."""
function has_augmentation(el::Element)
    has_non_local(el) && !isnothing(el.potential.non_local_potential.augmentation)
end

"""Check presence of (pseudo-)atomic orbitals."""
has_orbitals(el::Element) = !isnothing(el.potential.orbitals)
