...
@timing function write_w90_unk(fileprefix::String, basis, ψ; n_bands, spin=1)
    ...
    if spin == 2
        ik_shift = length(basis.kpoints) ÷ 2
    else
        ik_shift = 0
    end

    for ik in krange_spin(basis, spin)
        open(dirname(fileprefix) * (@sprintf "/UNK%05i.%i" ik - ik_shift spin), "w") do fp
            println(fp, "$(fft_size[1]) $(fft_size[2]) $(fft_size[3]) $(ik-ik_shift) $n_bands")
...
@timing function write_w90_amn(fileprefix::String, basis::PlaneWaveBasis, ψ; n_bands, centers)
    open(fileprefix * ".amn", "w") do fp
        println(fp, "Generated by DFTK at $(now())")
        println(fp, "$n_bands   $(length(ψ))  $(length(centers))")
...
@timing function write_w90_omn(fileprefix::String, basis::PlaneWaveBasis, ψ; n_bands=size(ψ[1], 2))
    @assert basis.model.spin_polarization == :collinear
    n_kpts = length(basis.kpoints) ÷ 2

    open(fileprefix * ".omn", "w") do fp
        println(fp, "Generated by DFTK at $(now())")
        println(fp, "$n_bands  $n_kpts  $n_bands")

        for (ik, (ψk, kpt)) in enumerate(zip(ψ[1:n_kpts], basis.kpoints[1:n_kpts]))
            ψk_dn = ψ[n_kpts+ik][:, 1:n_bands]
            # I think spin up & dn have the same kpt coordinates, so the
            # G vectors should be the same.
            Ok = ψk[:, 1:n_bands]' * ψk_dn
            for n in 1:n_bands
                for m in 1:n_bands
                    @printf(fp, "%3i %3i %3i  %22.18f %22.18f \n",
                            m, n, ik, real(Ok[m, n]), imag(Ok[m, n]))
                end
            end
        end
    end
...
@timing function run_wannier90(scfres;
    ...
    if basis.model.spin_polarization == :collinear
        fileprefix_up = fileprefix * "_up"
        nk = length(scfres.basis.kpoints) ÷ 2
        write_w90_eig(fileprefix_up, scfres_unfold.eigenvalues[1:nk]; n_bands)
        write_w90_amn(fileprefix_up, basis, ψ[1:nk]; n_bands, centers)
        write_w90_mmn(fileprefix_up, basis, ψ[1:nk], nnkp; n_bands)
        fileprefix_dn = fileprefix * "_dn"
        write_w90_eig(fileprefix_dn, scfres_unfold.eigenvalues[nk+1:end]; n_bands)
        write_w90_amn(fileprefix_dn, basis, ψ[nk+1:end]; n_bands, centers)
        write_w90_mmn(fileprefix_dn, basis, ψ[nk+1:end], nnkp; n_bands)
    else
    ...
    if wannier_plot
        if basis.model.spin_polarization == :collinear
            fileprefix_up = fileprefix * "_up"
            nk = length(basis.kpoints) ÷ 2
            write_w90_unk(fileprefix_up, basis, ψ; n_bands, spin=1)
            fileprefix_dn = fileprefix * "_dn"
            write_w90_unk(fileprefix_dn, basis, ψ; n_bands, spin=2)
        else
