...
write_win(prefix::String, scfres, num_bands, num_wann; bands_plot=false, kwargs...) =
    write_win(prefix, scfres.basis, scfres.ψ, num_bands,
              num_wann; bands_plot=bands_plot, kwargs...)
...
"""
function write_eig(prefix::String, scfres, num_bands::Integer)
    open("$prefix.eig", "w") do f
        for (k, e_k) in enumerate(scfres.eigenvalues)
            for (n, e_kn) in enumerate(e_k[1:num_bands])
                write(f,@sprintf("%3i  %3i   %25.18f \n", n, k,
                                 auconvert(Unitful.eV, e_kn).val))
            end
        end
    end
    return nothing
...
function overlap_Mmn_k_kpb(basis::PlaneWaveBasis, ψ, ik::Integer, ikpb::Integer,
    ...
    Mkb = zeros(ComplexF64, (num_bands, num_bands))
    ...
    # TMP FIX
    (iszero(Mkb)) && (return diagm(ones(num_bands))) # Avoir NAN in Wannierization.
    Mkb
end
...
"""
function write_mmn(prefix::String, basis::PlaneWaveBasis, ψ, num_bands::Integer,
    ...
    open("$prefix.mmn", "w") do f
        ...
        write(f, "$num_bands  $k_size  $nn_num\n")
        ...
        for i_nnkp in 1:size(nn_kpts, 1)
            ...
            # The matrix itself
            Mkb = overlap_Mmn_k_kpb(basis, ψ, k, kpb, num_bands, G_shift=shift)
            for ovlp in Mkb
...
write_mmn(prefix::String, scfres, num_bands, nn_kpts, nn_num) =
    write_mmn(prefix, scfres.basis, scfres.ψ, num_bands, nn_kpts, nn_num)
...
function compute_Ak_gaussian_guess(basis::PlaneWaveBasis, ψ_k, k_pt,
                                     num_bands::Integer, num_wann::Integer;
                                     centers=[])
...
"""
function write_amn(prefix::String, basis::PlaneWaveBasis, ψ, num_bands::Integer,
                   num_wann::Integer; centers=[])
    ...
    open("$prefix.amn", "w") do f
        # Header
        write(f, "Generated by DFTK at $(now())"*"\n")
        write(f, "$num_bands   $k_size   $num_wann \n")

        # Matrices
        for k in 1:k_size
            ψ_k = ψ[k]; k_pt = basis.kpoints[k];
            Ak = compute_Ak_gaussian_guess(basis, ψ_k, k_pt, num_bands, num_wann;
                              centers=centers)
...
write_amn(prefix, scfres, num_bands, num_wann; centers=[]) =
    write_amn(prefix, scfres.basis, scfres.ψ, num_bands, num_wann, centers=centers)
