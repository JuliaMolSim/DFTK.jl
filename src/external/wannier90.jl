...
function apply_U_matrices(file, ψ)
    ...
    ψ_out = copy(ψ) # Better ideas than copy to inherit the type of ψ in ψ_out ?
    for k in 1:num_kpts
        # Locate the beginning of the matrix Uk written as a vector
        i_Uk = 4 + (k-1)*(len_Uk+1)
        Uk = reshape([x+im*y for (x,y) in eachrow(Uks[i_Uk:i_Uk+(len_Uk-1), 1:2])],
                     num_ψin, num_ψout)
        # Check that Uk is unitary
        (norm(Uk'Uk - diagm(ones(num_ψout))) > 1e-8) && (@error("U matrix is not unitary"))
        # Compute all ψ_out[k][:,n]
        ψ_out_k = zero(ψ[k][:,1:num_ψout])
        for n in 1:num_ψout
            ψ_out_k[:,n] = ψ[k][:,1:num_ψin]*Uk[:,n]
        end
        ψ_out[k] = ψ_out_k
    end
    ψ_out
...
@doc raw"""
    Wraps disentanglement (if needed) and wannierization to return Wanniers function
    as a table ``W[k][G,n]``
    The prefix in argument is the one returned by the `run_wannier90` routine above.
    `ψ_unfold` is the array of Bloch waves from an "unfolded" self-consistent-field result.
"""
function extract_wannier_functions(prefix, ψ_unfold)
...
function extract_wannier_functions(prefix, scfres::NamedTuple)
    scfres_unfold = unfold_bz(scfres);
    extract_wannier_functions(prefix, scfres_unfold.ψ)
end
...
"""
function plot_wannier_function(basis::PlaneWaveBasis, w_fourier, n_band)
    ...
    vtk_grid("wannier_$(n_band)", x, y, z) do vtk
...
plot_wannier_function(scfres::NamedTuple, prefix::String, n_band) =
    plot_wannier_function(unfold_bz(scfres.basis), extract_wannier_functions(prefix, scfres), n_band)
