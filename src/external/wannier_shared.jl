...
@timing function write_w90_unk(fileprefix::String, basis, ψ; n_bands, spin=1)
    ...
    for ik in krange_spin(basis, spin)
        open(dirname(fileprefix) * (@sprintf "/UNK%05i.%i" ik spin), "w") do fp
            ...
            for iband = 1:n_bands
                ψnk_real = ifft(basis, basis.kpoints[ik], @view ψ[ik][1, :, iband])
...
@views function overlap_Mmn_k_kpb(basis::PlaneWaveBasis, ψ, ik, ik_plus_b, G_shift, n_bands)
    ...
    for n = 1:n_bands
        for m = 1:n_bands
            ...
            Mkb[m, n] = dot(ψ[ik][1, ip, m], ψ[ik_plus_b][1, ip_plus_b, n])
        end
    end
...
@timing function write_w90_amn(
    ...
    open(fileprefix * ".amn", "w") do fp
        ...
        for (ik, (ψk, kpt)) in enumerate(zip(ψ, basis.kpoints))
            Ak = compute_amn_kpoint(basis, kpt, ψk[1, :, :], projections, n_bands)
            for n = 1:size(Ak, 2)
                for m = 1:size(Ak, 1)
