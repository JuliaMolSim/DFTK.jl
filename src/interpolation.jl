...
function interpolate_kpoint(data_in::AbstractVecOrMat, kpoint_in::Kpoint, kpoint_out::Kpoint)
    ...
    data_newk = similar(data_in, length(G_vectors(kpt_new)), n_bands) .= 0
    for (iold, inew) in enumerate(indexin(G_vectors(kpoint_in), G_vectors(kpt_new)))
        inew !== nothing && (data_newk[inew, :] = data_in[iold, :])
    end
    data_newk
end
