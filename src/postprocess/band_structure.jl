...
@timing function compute_bands(
    ...
    ksymops = [[identity_symop()] for _ = 1:length(kcoords)]
    ...
    band_data = diagonalize_all_kblocks(
        eigensolver,
        ham,
        n_bands + 3;
...
function prepare_band_data(
    ...
    for key in datakeys
        ...
        ret[key] = [
            Dict(spinsym => data["energy"][ibranch][spin] for (spin, spinsym) in spinmap) for ibranch = 1:length(data["energy"])
        ]
    end
...
function is_metal(band_data, εF, tol = 1e-4)
    ...
    for ib = 1:n_bands
        some_larger = any(band_data.λ[ik][ib] - εF < -tol for ik = 1:n_kpoints)
        some_smaller = any(εF - band_data.λ[ik][ib] < -tol for ik = 1:n_kpoints)
...
function detexify_kpoint(string)
    ...
    replacements = ("\\Gamma" => "Γ", "\\Delta" => "Δ", "\\Sigma" => "Σ", "_1" => "₁")
...
function plot_bandstructure(
    ...
    kcoords, klabels, kpath =
        high_symmetry_kpath(basis.model; kline_density = kline_density)
