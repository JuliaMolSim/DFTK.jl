...
function next_density(
    ...
    if ψ !== nothing
        ...
        for ik = 1:length(ham.basis.kpoints)
            @assert size(ψ[ik], 2) == n_bands + n_ep_extra
        end
    end

    # Diagonalize
    eigres = diagonalize_all_kblocks(
        eigensolver,
        ham,
        n_bands + n_ep_extra;
        guess = ψ,
...
@timing function self_consistent_field(
    ...
    function fixpoint_map(x)
        ...
        if n_iter == 1 # first iteration
            ...
            energies, ham = energy_hamiltonian(
                basis,
                ψ,
                occupation;
                ρ = ρin,
        ...
        nextstate = next_density(
            ham;
            n_bands = n_bands,
            ψ = ψ,
            eigensolver = eigensolver,
            miniter = 1,
            tol = determine_diagtol(info),
    ...
    info = (
        ...
        n_iter = n_iter,
        ψ = ψ,
        diagonalization = info.diagonalization,
        stage = :finalize,
    )
    callback(info)
    info
