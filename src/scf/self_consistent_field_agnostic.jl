...
"""
function next_state(ham::Hamiltonian,
...
- `callback`: Function called at each SCF iteration. Usually takes care of printing the
  intermediate state.
"""
@timing function self_consistent_field_agnostic(
    ...
    function fixpoint_map(statein, info)
        (; ψ, occupation, eigenvalues, εF, n_iter, converged, timedout, τ, hubbard_n) = merge(return_tuple(statein), info)
        ρin = statein.ρ
        ...
        nextstate = next_state(ham, nbandsalg, fermialg; eigensolver, ψ, eigenvalues,
        ...
        callback(info_next)
        rearrange(statetype, ρnext, info_next)
    end
    ...
    state_init, info_init = rearrange(statetype, ρ, info_init)
    # Convergence is flagged by is_converged inside the fixpoint_map.
    state, info = solver(fixpoint_map, state_init, info_init; maxiter)
    ...
    (; ρin, ρout, τ, hubbard_n, ψ, occupation, eigenvalues, εF, converged) = merge(return_tuple(state), info)
    energies, ham = energy_hamiltonian(basis, ψ, occupation; 
                                       exxalg=VanillaExx(),
