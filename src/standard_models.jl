...
function model_atomic(lattice::AbstractMatrix, atoms::Vector; extra_terms=[], kwargs...)
    ...
    Model(lattice; name="atomic", atoms=atoms, terms=terms, kwargs...)
end
...
function model_DFT(lattice::AbstractMatrix, atoms::Vector, xc::Xc; extra_terms=[], kwargs...)
    name = isempty(xc.functionals) ? "rHF" : join(xc.functionals, "+")
    model_atomic(lattice, atoms; extra_terms=[Hartree(), xc, extra_terms...], name, kwargs...)
end
