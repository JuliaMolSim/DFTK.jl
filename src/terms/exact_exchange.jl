...
function (ex::ExactExchange)(basis)
    TermExactExchange(basis, ex.scaling_factor, ex.interaction_kernel, ex.exx_algorithm)
end

struct TermExactExchange <: Term
    scaling_factor::Real  # scaling factor, absorbed into fourier_kernel
    fourier_kernel::AbstractArray
    exx_algorithm::ExxAlgorithm
end
function TermExactExchange(basis::PlaneWaveBasis{T}, scaling_factor, interaction_kernel, exx_algorithm) where {T}
    # TODO: we need this for every q-point
    fac::T = scaling_factor
    fourier_kernel = fac .* compute_fourier_kernel(basis; interaction_kernel)
    TermExactExchange(fac, fourier_kernel, exx_algorithm)
end
...
@timing "ene_ops: ExactExchange" function ene_ops(term::TermExactExchange, basis::PlaneWaveBasis{T},
    ...
    for (ik, kpt) in enumerate(basis.kpoints)
        ...
        (; Ek, op) = exx_operator(term.exx_algorithm, basis, kpt,
                                  term.fourier_kernel, ψk, ψk_real, occk)
        push!(ops, op)
...
function exx_operator(::VanillaExx, basis::PlaneWaveBasis{T}, kpt, fourier_kernel,
                      ψk, ψk_real, occk) where {T}
    Ek = exx_energy_only(basis, kpt, fourier_kernel, ψk_real, occk)
    op = ExchangeOperator(basis, kpt, fourier_kernel, occk, ψk, ψk_real)
    (; Ek, op)
end
...
function exx_operator(::AceExx, basis::PlaneWaveBasis{T}, kpt, fourier_kernel::AbstractArray,
                      ψk, ψk_real, occk) where {T}
    ...
    Vx = ExchangeOperator(basis, kpt, fourier_kernel, occk, ψk, ψk_real)
...
function exx_energy_only(basis::PlaneWaveBasis{T}, kpt, fourier_kernel, ψk_real, occk) where {T}
    ...
    for (n, ψnk_real) in enumerate(eachslice(ψk_real, dims=4))
        for (m, ψmk_real) in enumerate(eachslice(ψk_real, dims=4))
            ...
            Ek -= 1/T(2) * fac_mn * real(dot(ρmn_fourier .* fourier_kernel, ρmn_fourier))
        end
    end
    Ek
end
