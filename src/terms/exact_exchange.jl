...
struct TermExactExchange <: Term
    scaling_factor::Real  # scaling factor, absorbed into kernel_fourier
    kernel_fourier::AbstractArray
...
function TermExactExchange(basis::PlaneWaveBasis{T}, scaling_factor, interaction_kernel, exx_algorithm) where {T}
    ...
    kernel_fourier = fac .* compute_kernel_fourier(basis; interaction_kernel)
    TermExactExchange(fac, kernel_fourier, exx_algorithm)
end
...
@timing "ene_ops: ExactExchange" function ene_ops(term::TermExactExchange, basis::PlaneWaveBasis{T},
    ...
    for (ik, kpt) in enumerate(basis.kpoints)
        ...
        (; Ek, op) = exx_operator(term.exx_algorithm, basis, kpt,
                                  term.kernel_fourier, ψk, ψk_real, occk)
        push!(ops, op)
...
function exx_operator(::VanillaExx, basis::PlaneWaveBasis{T}, kpt, kernel_fourier,
                      ψk, ψk_real, occk) where {T}
    Ek = exx_energy_only(basis, kpt, kernel_fourier, ψk_real, occk)
    op = ExchangeOperator(basis, kpt, kernel_fourier, occk, ψk, ψk_real)
    (; Ek, op)
end
...
function exx_operator(::AceExx, basis::PlaneWaveBasis{T}, kpt, kernel_fourier::AbstractArray,
                      ψk, ψk_real, occk) where {T}
    ...
    Vx = ExchangeOperator(basis, kpt, kernel_fourier, occk, ψk, ψk_real)
...
function exx_energy_only(basis::PlaneWaveBasis{T}, kpt, kernel_fourier, ψk_real, occk) where {T}
    ...
    for (n, ψnk_real) in enumerate(eachslice(ψk_real, dims=4))
        for (m, ψmk_real) in enumerate(eachslice(ψk_real, dims=4))
            ...
            Ek -= 1/T(2) * fac_mn * real(dot(ρmn_fourier .* kernel_fourier, ρmn_fourier))
        end
    end
    Ek
end
