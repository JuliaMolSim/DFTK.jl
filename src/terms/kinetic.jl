...
term_name(term::TermKinetic) = "Kinetic"

function ene_ops(term::TermKinetic, ψ, occ; kwargs...)
    basis = term.basis
    T = eltype(basis)

    ops = [FourierMultiplication(basis, kpoint, term.kinetic_energies[ik]) for (ik, kpoint) in enumerate(basis.kpoints)]
    ...
    for (ik, k) in enumerate(basis.kpoints)
        for iband = 1:size(ψ[1], 2)
            ...
            E += basis.kweights[ik] * occ[ik][iband] * real(dot(ψnk, term.kinetic_energies[ik] .* ψnk))
        end
    end
