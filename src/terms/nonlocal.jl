...
function (::AtomicNonlocal)(basis::PlaneWaveBasis)
    ...
    atoms = [elem.psp => positions for
    (elem, positions) in basis.model.atoms if elem isa ElementPsp]
...
function ene_ops(term::TermAtomicNonlocal, ψ, occ; kwargs...)
    ...
    for (ik, k) in enumerate(basis.kpoints)
        for iband = 1:size(ψ[1], 2)
            ...
            E +=
                basis.kweights[ik] * occ[ik][iband] * real(dot(Pψnk, term.ops[ik].D * Pψnk))
        end
    end
            ...
            for α = 1:3
                ...
                for (ik, kpt_irred) in enumerate(basis.kpoints)
                    ...
                    for isym = 1:length(basis.ksymops[ik])
                        ...
                        qs = [
                            basis.model.recip_lattice * (Skcoord + G)
                            for G in G_vectors(Skpoint)
                        ]
                        form_factors = build_form_factors(el.psp, qs)
                        structure_factors =
                            [cis(-2T(π) * dot(Skcoord + G, r)) for G in G_vectors(Skpoint)]
                        P = structure_factors .* form_factors ./ sqrt(unit_cell_volume)
                        dPdR =
                            [-2T(π) * im * (Skcoord+G)[α] for G in G_vectors(Skpoint)] .* P

                        dHψSk = P * (C * (dPdR' * ψSk))
                        for iband = 1:size(ψ[ik], 2)
                            @views fr[α] -= (
                                occ[ik][iband] / tot_red_kpt_number *
...
function count_n_proj_(psp::PspHgh)
    psp.lmax < 0 ? 0 : sum(size(psp.h[l+1], 1) * (2l + 1) for l = 0:psp.lmax)::Int
end
...
function build_projection_coefficients_(basis::PlaneWaveBasis{T}, atoms) where {T}
    ...
    for (psp, positions) in atoms, r in positions
        ...
        proj_coeffs[count+1:count+n_proj_psp, count+1:count+n_proj_psp] =
            build_projection_coefficients_(psp)
        count += n_proj_psp
    end # psp, r
    @assert count == n_proj
...
function build_projection_vectors_(basis::PlaneWaveBasis{T}, atoms, kpt::Kpoint) where {T}
    ...
    for (psp, positions) in atoms
        ...
        for r in positions
            ...
            structure_factors =
                [cis(-2T(π) * dot(kpt.coordinate + G, r)) for G in G_vectors(kpt)]
            for iproj = 1:count_n_proj_(psp)
                @views proj_vectors[:, offset+iproj] .= (
                    structure_factors .* form_factors[:, iproj] ./
