    ...
    for (psp, positions) in atoms
        ...
        for r in positions
            ...
            for iproj = 1:count_n_proj(psp)
                col = structure_factors .* form_factors[:, iproj] ./ sqrt(unit_cell_volume)
                proj_vectors = length(proj_vectors)>0 ? hcat(proj_vectors,col) : col
            end
...
function _build_form_factors(psp, qs)
    qnorms = norm.(qs)
    T = real(eltype(qnorms))
    # Compute position-independent form factors
    #form_factors = zeros(Complex{T}, length(qs), count_n_proj(psp))
    form_factors = zeros(Complex{T}, 0, 0)
    count = 1
    for l in 0:psp.lmax, m in -l:l
        ...
        for iproj in 1:n_proj_l
            ...
            form_factor_i = prefac_lm .* radial_il
            form_factors = length(form_factors) > 0 ? hcat(form_factors, form_factor_i) : form_factor_i
            count += 1
        end
    end
