...
function build_projection_coefficients_(T, psps, psp_positions)
    ...
    n_proj = PseudoPotentialIO.n_projector_angulars(psps, psp_positions)
    proj_coeffs = zeros(T, n_proj, n_proj)

    count = 0
    for (psp, positions) in zip(psps, psp_positions), _ in positions
        n_proj_psp = PseudoPotentialIO.n_projector_angulars(psp)
...
function build_projection_coefficients_(T, psp)
    n_proj = PseudoPotentialIO.n_projector_angulars(psp)
    proj_coeffs = zeros(T, n_proj, n_proj)
    count = 0
    for l in PseudoPotentialIO.angular_momenta(psp), m in -l:l
        n_proj_l = PseudoPotentialIO.n_projector_radials(psp, l)  # Number of i's
        range = count .+ (1:n_proj_l)
        proj_coeffs[range, range] = PseudoPotentialIO.projector_coupling(psp, l)
        count += n_proj_l
    end # l, m
    proj_coeffs
end
...
function build_projection_vectors_(basis::PlaneWaveBasis{T}, kpt::Kpoint,
    ...
    for (psp, positions) in zip(psps, psp_positions)
        ...
        for r in positions
            ...
            @views for iproj = 1:PseudoPotentialIO.n_projector_angulars(psp)
                proj_vectors[:, offset+iproj] .= (
                    structure_factors .* form_factors[:, iproj] ./ sqrt(unit_cell_volume)
                )
            end
            offset += PseudoPotentialIO.n_projector_angulars(psp)
        end
    end
...
function build_form_factors(psp, qs::Array)
    ...
    n_proj_max = maximum(l -> PseudoPotentialIO.n_projector_radials(psp, l), PseudoPotentialIO.angular_momenta(psp); init=0)
    ...
    for q in qs
        ...
        if !haskey(radials, q_norm)
            ...
            for l in PseudoPotentialIO.angular_momenta(psp), iproj_l in 1:PseudoPotentialIO.n_projector_radials(psp, l)
                β̃nl = PseudoPotentialIO.projector_fourier(psp, l, iproj_l)
                radials_q[iproj_l, l+1] = β̃nl(q_norm)
            end
            radials[q_norm] = radials_q
        end
    end

    form_factors = Matrix{Complex{T}}(undef, length(qs), PseudoPotentialIO.n_projector_angulars(psp))
    for (iq, q) in enumerate(qs)
        radials_q = radials[norm(q)]
        count = 1
        for l in PseudoPotentialIO.angular_momenta(psp), m in -l:l
            angular = im^l * ylm_real(l, m, q)
            for iproj_l in 1:PseudoPotentialIO.n_projector_radials(psp, l)
                form_factors[iq, count] = radials_q[iproj_l, l+1] * angular
                count += 1
            end
        end
        @assert count == PseudoPotentialIO.n_projector_angulars(psp) + 1
    end
    form_factors
end
