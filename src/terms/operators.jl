...
function LinearAlgebra.mul!(Hψ::AbstractVector, op::RealFourierOperator, ψ::AbstractVector)
    ψ_real = ifft(op.basis.fft_bundle, op.kpoint, ψ)
    ...
    Hψ .= Hψ_fourier .+ fft(op.basis.fft_bundle, op.kpoint, Hψ_real)
    Hψ
end
...
function Matrix(op::RealSpaceMultiplication)
    ...
    pot_fourier = fft(op.basis.fft_bundle, op.potential)
...
function apply!(Hψ, op::MagneticFieldOperator, ψ)
    ...
    for α = 1:3
        ...
        ∂αψ_real = ifft(op.basis.fft_bundle, op.kpoint, ∂αψ_fourier)
...
function apply!(Hψ, op::DivAgradOperator, ψ;
    ...
    for α = 1:3
        ∂αψ_real = ifft!(ψ_scratch, op.basis.fft_bundle, op.kpoint, im .* G_plus_k[α] .* ψ.fourier)
        A∇ψ      = fft(op.basis.fft_bundle, op.kpoint, ∂αψ_real .* op.A)
