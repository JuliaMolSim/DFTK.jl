...
@timing_seq "apply RealSpaceMultiplication" function apply!(
    Hψ,
    op::RealSpaceMultiplication,
    ψ,
)
...
function Matrix(op::RealSpaceMultiplication)
    ...
    for i = 1:npw
        ...
        for j = 1:npw
            ...
            if ind === nothing
                error(
                    "For full matrix construction, the FFT size must be " *
...
struct MagneticFieldOperator{T<:Real} <: RealFourierOperator
    ...
    kpoint::Kpoint{T}
    Apot::Any  # Apot[α][i,j,k] is the A field in (cartesian) direction α
end
@timing_seq "apply MagneticFieldOperator" function apply!(Hψ, op::MagneticFieldOperator, ψ)
    ...
    for α = 1:3
        ...
        pα = [
            (op.basis.model.recip_lattice*(G+op.kpoint.coordinate))[α]
            for G in G_vectors(op.kpoint)
        ]
...
function optimize_operators_(ops)
    ...
    combined_RSmults = RealSpaceMultiplication(
        RSmults[1].basis,
        RSmults[1].kpoint,
