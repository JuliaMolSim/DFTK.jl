...
@testitem "Symmetries broken by perturbation are filtered out" tags=[:dont_test_mpi] begin
    ...
    is_approx_in(collection, s1) = any(s -> isapprox(s, s1), collection)

    # Generate the full group
    function complete_group(symops)
        completed_group = Vector(symops)

        for it = 1:10
            if it == 10
                error("Could not complete group in 10 iterations")
            end
            to_add = []
            function check(s1)
                if !is_approx_in(completed_group, s1) && !is_approx_in(to_add, s1)
                    push!(to_add, s1)
                end
            end
            ...
            push!(completed_group, to_add...)
        end
        DFTK.check_group(completed_group)
    end

    symmetries_full = complete_group([rotyz, mirrory, transx])
    ...
    function check_symmetries(filtered_symmetries, expected_symmetries)
        expected_symmetries = complete_group(expected_symmetries)

        @test length(filtered_symmetries) == length(expected_symmetries)
        for s in expected_symmetries
            @test is_approx_in(filtered_symmetries, s)
        end
        for s in filtered_symmetries
            @test is_approx_in(expected_symmetries, s)
        end
    end
    ...
    @testset "Atom movement" begin
        ...
        symmetries_filtered = DFTK.filter_out_dual_symmetries(lattice, atoms, positions_modified, symmetries_full)
    ...
    @testset "Lattice strain" begin
        ...
        symmetries_filtered = DFTK.filter_out_dual_symmetries(lattice_modified, atoms, positions, symmetries_full)
    ...
    @testset "Atom movement + lattice strain" begin
        ...
        positions_modified = [[0., 0., 0.], [0.5 + ε, 0., 0.]]
        lattice_modified = diagm([2., 1., 1. + ε])
        symmetries_filtered = DFTK.filter_out_dual_symmetries(lattice_modified, atoms, positions_modified, symmetries_full)
...
@testitem "Symmetry-breaking perturbation using ForwardDiff" #=
    ...
    function run_scf(ε)
        ...
        pos = map(enumerate(aluminium.positions)) do (i, x)
            i == 1 ? x + ε * [1., 0, 0] : x
        end
        println(typeof(pos))
    ...
    @show norm(δρ)
    @show norm(δρ_finitediff)

    @test norm(δρ - δρ_finitediff) * sqrt(scfres0.basis.dvol) < 2e-7
end