...
function test_consistency_term(term; rtol=1e-4, atol=1e-8, ε=1e-6, kgrid=[1, 2, 3],
                               ...
                               atoms=silicon.atoms, positions=silicon.positions,
                               Ecut=10, spin_polarization=:none, integer_occupation=false,
                               test_matrixification=true
    )
    ...
    @testset "$(typeof(term))$xc $sspol" begin
        ...
        n_bands = div(model.n_electrons, 2, RoundUp)
        ...
        if test_matrixification
            @testset "Matrixification of operator" begin
                for ik in 1:length(basis.kpoints), operator in ham.blocks[ik].operators
                    mat_k = Matrix(ham.blocks[ik])
                    @test maximum(abs, mat_k - mat_k') < atol
                    @test norm(mat_k * ψ[ik] - operator * ψ[ik]) < atol
                end
            end
        end

        @testset "Operator is derivative of energy" begin
            ...
            for ik in 1:length(basis.kpoints)
                ...
                occk = occupation[ik]
                δψkHψk = sum(occk[iband] * real(dot(δψ[ik][:, iband], Hψk[:, iband]))
                             for iband=1:n_bands)
...
@testset "Hamiltonian consistency" begin
    ...
    let
        ...
        positions = [0.5ones(3)]
        atoms     = [ElementCoulomb(:Be)]
        test_consistency_term(ExactExchange();
                              lattice, positions, atoms,
                              kgrid=(1, 1, 1), Ecut=7,
                              integer_occupation=true)
    end
    test_consistency_term(ExactExchange();
                          kgrid=(1, 1, 1), Ecut=7,
                          integer_occupation=true)
