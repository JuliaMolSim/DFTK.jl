using Test
using DFTK: smearing_functions, find_fermi_level, find_occupation, Model, PlaneWaveModel
using DFTK: find_occupation_gap_zero_temperature, find_occupation_metal
...
@testset "Smearing for insulators" begin
    ...
    for T in Ts, fun in smearing_functions
        ...
        basis = PlaneWaveModel(model, fft_size, Ecut, silicon.kcoords, silicon.kweights, silicon.ksymops)
        _, occs = find_occupation_metal(basis, energies, Psi)
    ...
    for T in Ts, fun in smearing_functions
        model = Model(silicon.lattice, silicon.n_electrons; temperature=T, smearing=fun)
        basis = PlaneWaveModel(model, fft_size, Ecut, silicon.kcoords, silicon.kweights, silicon.ksymops)
        _, occupation = find_occupation_metal(basis, energies, Psi)
