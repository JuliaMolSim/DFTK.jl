...
function run_oxygen_pbe(T; kwargs...)
    ...
    # thus require the same discretisation parameters to be obtained.
    ref_evals = [
    ...
    scfres = run_scf_and_compare(T, basis, ref_evals, ref_etot; ρspin=ρspin,
                                 test_etot=false, kwargs...)
    ...
    @test magnetisation ≈ ref_magn atol=5e-6
end

@testset "Oxygen PBE (Float64)" begin
    run_oxygen_pbe(Float64, test_tol=5e-6, scf_tol=1e-8, n_ignored=1)
end
