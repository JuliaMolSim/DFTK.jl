@testitem "Phonon: Symmetry transformation of dynamical matrix" #=
    ...
    # Choose a q-point not at high symmetry
    q = Vec3([0.25, 0.0, 0.0])
    ...
    if !isnothing(symop_test)
        ...
        dynmat_Sq_direct = DFTK.compute_dynmat(scfres, q_symm; tol=1e-8)
        ...
        @test dynmat_Sq_direct_cart ≈ dynmat_Sq_symm_cart rtol=1e-4
        ...
        @test modes_direct.frequencies ≈ modes_symm.frequencies rtol=1e-4
    else
