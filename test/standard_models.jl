@testitem "Standard DFT model construction" setup=[TestCases] tags=[:minimal] begin
    ...
    @testset "lda from symbols" begin
        ...
        @test length(dftterms) == 1
        @test dftterms[1] isa Xc
        @test dftterms[1].functionals == [:lda_x, :lda_c_pw]
    ...
    @testset "xc object plus symbol is error" begin
        ...
        @test_throws model_DFT(system; functionals=[xc, :lda_x]) ArgumentError
    end

    @testset "HF is error" begin
        exx = ExactExchange()
        @test_throws model_DFT(system; functionals=[exx]) ArgumentError
    end
    ...
    @testset "hybrid with implicit exx" begin
        ...
        @test dftterms[1] isa Xc
        @test dftterms[1].functionals == [:hyb_gga_xc_pbeh]
    ...
    @testset "pbe0 with exx parameters" begin
        pbe0 = PBE0(; potential_threshold=1e-2, coulomb_kernel_model=SphericallyTruncated())
        ...
        @test dftterms[1].potential_threshold == 1e-2
        @test dftterms[2] isa ExactExchange
        @test dftterms[2] == ExactExchange(; coulomb_kernel_model=SphericallyTruncated())
    end
end
