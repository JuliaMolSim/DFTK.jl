...
silicon = (
    ...
    n_electrons = 8,
    temperature = 0.0,
    psp_hgh = PseudoPotentialIO.load_psp(hgh_lda_family, "si-q4.hgh"),
    psp_upf = PseudoPotentialIO.load_psp(pd_lda_family, "Si.upf"),
...
silicon = merge(silicon,
                (; atoms=fill(ElementPsp(silicon.atnum, psp=silicon.psp_hgh), 2)))
...
magnesium = (
    ...
    atnum = 12,
    n_electrons = 4,
    psp_hgh = PseudoPotentialIO.load_psp(hgh_lda_family, "mg-q2.hgh"),
    psp_upf = PseudoPotentialIO.load_psp(pd_lda_family, "Mg.upf"),
...
magnesium = merge(magnesium,
                  (; atoms=fill(ElementPsp(magnesium.atnum, psp=magnesium.psp_hgh), 2)))
...
aluminium_primitive = merge(aluminium_primitive,
                            (; atoms=fill(ElementPsp(aluminium_primitive.atnum,
                                                     psp=aluminium_primitive.psp_hgh), 1)))
...
platinum_hcp = (
    ...
    atnum = 78,
    n_electrons = 36,
    psp_hgh = PseudoPotentialIO.load_psp(hgh_lda_family, "pt-q18.hgh"),
    psp_upf = PseudoPotentialIO.load_psp(pd_lda_family, "Pt.upf"),
    positions = [zeros(3), ones(3) / 3],
    temperature = 0.0009500431544769484,
)
platinum_hcp = merge(platinum_hcp,
                     (; atoms=fill(ElementPsp(platinum_hcp.atnum, psp=platinum_hcp.psp_hgh), 2)))

iron_bcc = (
    lattice = 2.71176 .* [[-1 1 1]; [1 -1  1]; [1 1 -1]],
    atnum = 26,
    n_electrons = 8,
    psp_hgh = PseudoPotentialIO.load_psp(hgh_lda_family, "fe-q8.hgh"),
    psp_upf = PseudoPotentialIO.load_psp(pd_lda_family, "Fe.upf"),
    positions = [zeros(3)],
    temperature = 0.01,
)
iron_bcc = merge(iron_bcc, (; atoms=[ElementPsp(iron_bcc.atnum, psp=iron_bcc.psp_hgh)]))

o2molecule = (
    lattice = diagm([6.5, 6.5, 9.0]),
    atnum = 8,
    n_electrons = 12,
    psp_hgh = PseudoPotentialIO.load_psp(hgh_lda_family, "O-q6.hgh"),
    psp_upf = PseudoPotentialIO.load_psp(pd_lda_family, "O.upf"),
    positions = 0.1155 * [[0, 0, 1], [0, 0, -1]],
    temperature = 0.02,
)
o2molecule = merge(o2molecule,
                   (; atoms=fill(ElementPsp(o2molecule.atnum, psp=o2molecule.psp_hgh), 2)))
